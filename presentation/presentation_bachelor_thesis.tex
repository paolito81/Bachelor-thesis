...
\begin{frame}{Motivazione astrofisica}
	\begin{itemize}
		...
		\item<1-> Lo studio del modello standard solare (SSM) richiede conoscenze sulla composizione chimica del Sole
		\item<2-> Analisi spettroscopiche della fotosfera solare costruiscono due modelli in contraddizione tra loro: LZ (fotosfera 3D) e HZ (fotosfera 1D)
	...
	\end{itemize}
	\uncover<4->{\centering
		\includegraphics[width=0.48\textwidth]{img/CNO_Cycle.png}}
\end{frame}
	%metter	
	...
	\begin{columns}
		...
		\begin{column}{0.63\textwidth}
			...
			\only<3-4>{\centering
				\includegraphics[width=0.6\textwidth]{img/bgo_3d.png}}
			\only<5->{\centering
				\includegraphics[width=0.70\textwidth]{img/electronics.png}}
		\end{column}
	\end{columns}
\end{frame}
	...
	L'obiettivo della tesi, incentrato sull'analisi dei dati, è quello di 	
		\begin{itemize}%mettere schematicamente, punto per efficienza e punto per energia
			\item<2-> \textbf{calibrare in energia} gli spettri gamma 
			\item<3-> \textbf{caratterizzare in efficienza} di rivelazione dei fotoni per i sei segmenti del BGO
		\end{itemize}
	
		\uncover<4->{L'efficienza ottenuta tramite l'analisi dei dati sperimentali è stata confrontata con simulazioni GEANT4.}
	\end{frame}
...
%			\end{itemize}
		...
		\begin{column}{0.45\textwidth}
				...
				\centering
				\only<1>{\includegraphics[width=0.9\textwidth]{img/plot0_empty.pdf}}
				\only<2->{\includegraphics[width=0.9\textwidth]{img/plot0.pdf}}
	...
	% This part will be revealed on the second slide (within the same frame)
		\begin{columns}
			\begin{column}{0.5\textwidth}
				\visible<4->{\includegraphics[width=\textwidth]{img/parameter.pdf}}
			\end{column}
			\begin{column}{0.5\textwidth}
				\visible<5->{\includegraphics[width=\textwidth]{img/trapezoid.pdf}}
			\end{column}
		\end{columns}
\end{frame}
...
\begin{frame}{Tempo morto}%solo tempo vivo, indicare per spicchio 4 quanto vale. (tempo morto anziché tempo vivo)
	\begin{itemize}
		...
		\item<1-> Il pulser ha frequenza nota e serve per stimare il tempo morto dell'elettronica
		%rimettere schema elettronica
		\item<2-> Viene mandato in ogni segmento del BGO
		\item Il segnale del pulser in ciascun segmento viene confrontato con quello a frequenza nota
		\item<3-> Per il segmento 4 il tempo morto è circa l'$\mathbf{1.3\%}$ 
	\end{itemize}
	%leva gli istogrammi
...
\begin{frame}{Simulazioni}
	\begin{itemize}
		\item<1-> Il setup sperimentale, le sorgenti radioattive e la reazione $^{14}$N(p,$\gamma$)$^{15}$O sono stati simulati in GEANT4
		\item<2-> Gli spettri ottenuti con queste simulazioni sono a risoluzione energetica infinita
		\item<3-> Questi sono stati analizzati applicando la risoluzione energetica ottenuta dagli spettri sperimentali
	\end{itemize}
\end{frame}
