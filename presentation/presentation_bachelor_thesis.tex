...
%			\item<2-> Sezioni d'urto molto piccole (nb-fb) implicano segnale atteso molto minore rispetto al rumore di fondo in superficie: vogliamo minimizzarlo
			...
			\end{equation}
			\item<5-> Con una $\sigma \approx 10^{-12}$ barn si ha un \emph{reaction rate} ($N_{reaz}/\Delta t$) che vale appena $1\div 10$ conteggi al giorno %implies misure sotterranee
		\end{itemize}
		...
		\begin{columns}%a sinistra italia, a destra laboratori
			...
			\begin{column}{0.6\textwidth}
				\uncover<6->{\centering
					\includegraphics[width=0.9\textwidth]{img/noise.png}}
			\end{column}
...
\begin{frame}{LUNA 400 kV}%spiegare a voce
	...
	\captionof{figure}{L'acceleratore LUNA.}
\end{frame}
...
%				\item Si tratta di uno scintillatore, ossia uno strumento che quando eccitato da radiazione ionizzante, ne assorbe l'energia depositata e la riemette sotto forma di fotoni
		...
		\begin{column}{0.45\textwidth}
			...
			\uncover<4->{\centering
				\includegraphics[width=0.7\textwidth]{img/tin.jpg}}
		\end{column}
	\end{columns}
\end{frame}
...
%			\end{itemize}
			\begin{figure}%chiamare secondo grafico con "Energy" anziché Energy calibration
				%Cambiare nome "BGO 4" anziché graph 4
				\centering
				\only<1->{\includegraphics[width=\linewidth]{img/ex1775.pdf}}
				\only<2>{\includegraphics[width=0.9\textwidth]{img/plot0_empty.pdf}}
				\uncover<3->{\includegraphics[width=0.9\textwidth]{img/plot0.pdf}}
		...
		\begin{column}{0.45\textwidth}
			\begin{figure}
				\centering
				\uncover<4->{\includegraphics[width=0.9\textwidth]{img/residues0.pdf}}
%				\only<4->{\includegraphics[width=0.9\textwidth]{img/plot0.pdf}}
				%TERZA GRAFICA PER FARE VEDERE I RISULTATI
			\end{figure}%chiamare conversione b e agiugngere a
			\uncover<5->{\begin{table}[ht]
					\centering
...
\begin{frame}{Efficienza}
	\begin{itemize}
		...
		\end{equation}
		\item<3-> L'attività è calcolata al momento della misurazione (sorgenti radioattive di attività nota con precisione dello 0.7\% %verificare).
	\end{itemize}
...
\begin{frame}{Tempo morto}%solo tempo vivo, indicare per spicchio 4 quanto vale. (tempo morto anziché tempo vivo)
	\begin{itemize}
		...
		\item Il tempo morto è ricavato dal TTree delle coincidenze, ossia dove lo spicchio del BGO registra i fotoni del pulser nello spettro dei canali%questo a voce
	\end{itemize}
	%leva gli istogrammi
...
\begin{frame}{Simulazioni}
	\begin{itemize}
		\item confronto tabelle e grafico%mettere tabelle confronto tra exp e sim (spicchio 4)
	\end{itemize}
\end{frame}
