...
\begin{frame}{Sfide sperimentali della misura diretta}
	\begin{itemize}
		...
		\end{equation}
		\item<3-> Con una $\sigma \approx 10^{-12}$ barn si ha un \emph{reaction rate} ($N_{reaz}/\Delta t$) che vale appena $1\div 10$ conteggi al giorno
		%A VOCE
...
\end{frame}
	...
	\begin{frame}{Astrofisica nucleare sotterranea}
		\begin{itemize}
			...
			\item<2-> Sezioni d'urto molto piccole implicano segnale atteso molto minore rispetto al rumore di fondo in superficie 
		\end{itemize}
		\uncover<3>{\centering
	...
	\begin{columns}
		\begin{column}{0.45\textwidth}
			\begin{itemize}
				...
				\item<3-> Il bersaglio solido è in TiN
...
\begin{frame}{Calibrazione in energia}
	\begin{itemize}
		\item Viene effettuata con due sorgenti radioattive, $^{137}$Cs e $^{60}$Co
	...
	\begin{columns}
		...
		\begin{column}{0.5\textwidth}
			\visible<2->{\begin{itemize}
					\item Ai punti delle risoluzioni si fitta la funzione:
				\end{itemize}
				\begin{equation*}
					f(E) = a + \dfrac{b}{\sqrt{E}}
			\end{equation*}}	
