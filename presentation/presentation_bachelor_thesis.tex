...
\setbeamerfont{title}{size=\huge} % Set title size
\setbeamerfont{author}{size=\Large} % Set author size
\setbeamerfont{institute}{size=\large} % Set institute size
\setbeamerfont{date}{size=\large} % Set date size
...
\begin{document}
	...
	\begin{frame}[plain, noframenumbering]{Sommario}
		\tableofcontents
	\end{frame}
...
\begin{column}{0.45\textwidth}
	\begin{itemize}
		\item<1-> Tipicamente $\sigma \approx 10^{-12}$ barn: si ha un \emph{reaction rate} ($N_{reaz}/\Delta t$) che vale appena $1\div 10$ conteggi al giorno \only<2->{$\implies$ misure sotterranee}
	\end{itemize}
	
	\only<1->{\centering
		\includegraphics[width=0.75\textwidth]{img/location.png}}
	
\end{column}
\begin{column}{0.55\textwidth}
	\only<1>{\centering
		\includegraphics[width=\textwidth]{img/labs.png}}
		\only<2->{\centering
	...
	\begin{columns}
		\begin{column}{0.37\textwidth}
			\begin{itemize}
				...
				\item Rivelatore di fotoni: BGO (molto efficiente, alta densità $7.13$ g/cm$^3$)
...
\begin{frame}{Calcolo della calibrazione}
	\begin{columns}%mettere foto istogramma spicchio 4 con due elementi
		\begin{column}{0.53\textwidth}% a voce
%			\begin{itemize}
